...
const TicketsIndexPage = () => {
  ...
  const getTickets = async () => {
    if(!loading) setLoading(true);
      ...
      const {data} = await axios.get<Ticket[]>("/GetAllTickets");
      setLoading(false);
      setTickets(data);
  };
  ...
  const TicketsContent = (
    <table className="w-full ">
      ...
      <tbody className="bg-sky-100" >
          {
            Tickets.map((ticket) => (
              <tr key={ticket.id} className='border-2 border-gray-200 h-12 hover:bg-sky-400 transition-all duration-200'>
                ...
                <td className="text-center" >{moment(ticket.time).format("YYYY-MM-DD|HH:MM:SS")}</td>
                <td className="text-center" >
                  <Link to={`/tickets/${ticket.id}`} className="bg-green-600 p-1 rounded-md mx-1" >
                    Details
                  </Link>
                  <Link to={`/tickets/edit/${ticket.id}`} className="bg-yellow-600 p-1 rounded-md mx-1 max-md:hidden" >
                    Edit
                  </Link>
